...
class KeyStoreHelperForStorage {
    companion object {
        ...
        public fun createKeys(context: Context, alias: String) {
            ...
            val spec =
                KeyPairGeneratorSpec.Builder(context) // You'll use the alias later to retrieve the key. It's a key
                    ...
                    .setSerialNumber(BigInteger.valueOf(Math.abs(alias.hashCode())
                        .toLong())) // Date range of validity for the generated pair.
                    .setStartDate(start.time).setEndDate(end.time).build()
            val kpGenerator = KeyPairGenerator.getInstance(SecurityConstants.TYPE_RSA,
            ...
            try {
                ...
                keyPairGenerator.initialize(KeyGenParameterSpec.Builder(alias,
                    ...
                    .setAlgorithmParameterSpec(RSAKeyGenParameterSpec(1024,
                        RSAKeyGenParameterSpec.F4)).setBlockModes(KeyProperties.BLOCK_MODE_CBC)
                    .setEncryptionPaddings(KeyProperties.ENCRYPTION_PADDING_RSA_PKCS1).setDigests(
                        KeyProperties.DIGEST_SHA256,
                        KeyProperties.DIGEST_SHA384,
                        KeyProperties.DIGEST_SHA512) // Only permit the private key to be used if the user authenticated
                    // within the last five minutes.
                    .setUserAuthenticationRequired(requireAuth).build())
        ...
        private val cipher: Cipher
            private get() = Cipher.getInstance(String.format("%s/%s/%s",
